...
export const scriptsAPI = {
  ...
  createTextScript: (data) => 
    api.post('/scripts/create-text', data),
  ...
  getDashboardStats: () => 
    api.get('/scripts/dashboard/stats'),
  
  getScriptMetadata: () => 
    api.get('/scripts/metadata'),
  
  getScriptsNeedingAttention: () => 
    api.get('/scripts/attention-required'),
  
  getAvailableDeals: () => 
    api.get('/scripts/available-deals'),
  
  getUserScripts: (params = {}) => 
    api.get('/scripts', { params }),
  
  getScriptsByStatus: (status) => 
    api.get(`/scripts/status/${status}`),
  
  getScriptById: (scriptId) => 
    api.get(`/scripts/${scriptId}`),
  
  getScriptAnalysis: (scriptId) => 
    api.get(`/scripts/${scriptId}/analysis`),
  ...
  updateScript: (scriptId, updateData) => 
    api.patch(`/scripts/${scriptId}`, updateData),
  ...
  updateCreatorNotes: (scriptId, notes) => 
    api.patch(`/scripts/${scriptId}/notes`, { notes }),
  
  updateScriptTags: (scriptId, tags) => 
    api.patch(`/scripts/${scriptId}/tags`, { tags }),
  ...
  regenerateScript: (scriptId) => 
    api.post(`/scripts/${scriptId}/regenerate`),
  
  getGenerationStatus: (scriptId) => 
    api.get(`/scripts/${scriptId}/generation-status`),
  
  createScriptVariation: (scriptId, variationData) => 
    api.post(`/scripts/${scriptId}/variations`, variationData),
  
  getScriptVariations: (scriptId) => 
    api.get(`/scripts/${scriptId}/variations`),

  // ============================================
  // DEAL CONNECTION ENDPOINTS
  // ============================================
  
  linkScriptToDeal: (scriptId, dealId) => 
    api.post(`/scripts/${scriptId}/link-deal/${dealId}`),
  
  unlinkScriptFromDeal: (scriptId) => 
    api.delete(`/scripts/${scriptId}/unlink-deal`),

  // ============================================
  // SEARCH AND UTILITY ENDPOINTS
  // ============================================
  
  searchScripts: (searchData) => 
    api.post('/scripts/search', searchData),
  ...
  getScriptsHealth: () => 
    api.get('/scripts/health'),
  
  getUploadLimits: () => 
    api.get('/scripts/debug/limits'),
  ...
  createScriptWithFile: (scriptData, file, inputType = 'file_upload') => {
    ...
    const endpoint = inputType === 'video_transcription' ? 
      '/scripts/create-video' : 
      '/scripts/create-file';
  ...
  pollGenerationStatus: async (scriptId, maxAttempts = 30, interval = 2000) => {
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      try {
        ...
        const { status, isGenerationComplete } = response.data.data;
