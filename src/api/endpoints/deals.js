...
export const dealsAPI = {
  ...
  // --------------- NEW UPDATED ROUTES ---------------
  patchDealData: (dealId, document) => api.patch(`/deals/${dealId}`, document),
  // --------------- NEW UPDATED ROUTES ---------------
  ...
  updateDeal: (dealId, data) => {
    ...
    if (data.campaignRequirements !== undefined)
      cleanData.campaignRequirements = data.campaignRequirements
  ...
  archiveDeal: (dealId, archived = true) => api.put(`/deals/${dealId}/archive`, { archived }),
  ...
  addCommunication: (dealId, data) => api.post(`/deals/${dealId}/communications`, data),

  // Get all communications for a deal
  getCommunications: (dealId) => api.get(`/deals/${dealId}/communications`),
  ...
  addDeliverable: (dealId, data) => api.post(`/deals/${dealId}/deliverables`, data),
  ...
  quickActions: {
    sendReminder: (dealId) => api.post(`/deals/${dealId}/actions/send-reminder`, {}),

    markAsUrgent: (dealId) => api.post(`/deals/${dealId}/actions/mark-urgent`, {}),

    requestApproval: (dealId) => api.post(`/deals/${dealId}/actions/request-approval`, {}),

    generateInvoice: (dealId) => api.post(`/deals/${dealId}/actions/generate-invoice`, {}),
  },

  // ==================== Pipeline Management ====================

  // Get pipeline overview with all stages
  getPipelineOverview: () => api.get('/deals/pipeline/overview'),

  // Get deals by specific stage
  getDealsByStage: (stage) => api.get(`/deals/pipeline/${stage}`),

  // Get deals that need attention
  getAttentionNeeded: () => api.get('/deals/attention'),

  // ==================== Brands Management ====================

  // Get all brands
  getBrands: () => api.get('/deals/brands'),

  // Get specific brand
  getBrand: (brandId) => api.get(`/deals/brands/${brandId}`),

  // Update brand information
  updateBrand: (brandId, data) => api.put(`/deals/brands/${brandId}`, data),

  // ==================== Templates ====================

  // Create deal template
  createTemplate: (data) => api.post('/deals/templates', data),

  // Get all templates
  getTemplates: () => api.get('/deals/templates'),

  // Update template
  updateTemplate: (templateId, data) => api.put(`/deals/templates/${templateId}`, data),

  // Delete template
  deleteTemplate: (templateId) => api.delete(`/deals/templates/${templateId}`),

  // Apply template to new deal
  applyTemplate: (templateId) => api.get(`/deals/templates/${templateId}/apply`),
  ...
  // Get deal insights
  getDealInsights: () => api.get('/deals/analytics/insights'),

  // Get deal summary
  getDealSummary: () => api.get('/deals/analytics/summary'),

  // Get conversion funnel
  getConversionFunnel: (params = {}) => api.get('/deals/analytics/funnel', { params }),

  // Get performance metrics
  getPerformanceMetrics: (params = {}) => api.get('/deals/analytics/performance', { params }),
  ...
  getDealMetadata: () => api.get('/deals/metadata'),

  // Get deal stages configuration
  getStages: () => api.get('/deals/metadata/stages'),

  // Get deal statuses
  getStatuses: () => api.get('/deals/metadata/statuses'),

  // Get priority levels
  getPriorities: () => api.get('/deals/metadata/priorities'),
  ...
  // Get saved filters
  getSavedFilters: () => api.get('/deals/filters'),

  // Save a filter
  saveFilter: (filter) => api.post('/deals/filters', filter),

  // Delete saved filter
  deleteFilter: (filterId) => api.delete(`/deals/filters/${filterId}`),
  ...
  // Get deal notes
  getNotes: (dealId) => api.get(`/deals/${dealId}/notes`),

  // Add activity log
  addActivity: (dealId, activity) => api.post(`/deals/${dealId}/activities`, activity),

  // Get deal activities
  getActivities: (dealId) => api.get(`/deals/${dealId}/activities`),
  ...
  // Get deal documents
  getDocuments: (dealId) => api.get(`/deals/${dealId}/documents`),

  // Delete document
  deleteDocument: (dealId, documentId) => api.delete(`/deals/${dealId}/documents/${documentId}`),
  ...
  getDealHistory: (dealId) => api.get(`/deals/${dealId}/history`),

  // Restore deleted deal
  restoreDeal: (dealId) => api.post(`/deals/${dealId}/restore`),
