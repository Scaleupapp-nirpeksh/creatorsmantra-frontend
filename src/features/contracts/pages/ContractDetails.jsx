...
const ContractDetails = ({ contractId: propContractId }) => {
  ...
  const loadActivityFeed = async () => {
    ...
    try {
      const response = await apiCall(`/contracts?contractId=${contractId}&limit=20`)

      if (response.success) {
        setActivityFeed(response.data.contracts || [])
      } else {
  ...
  // TEMP
  // REASON: Not consu,ing anywhere in FE
  ...
  useEffect(() => {
    if (contractId) {
      ...
      Promise.all([
        fetchContract(),
        // OBSERVATIONS:
        // 1. No endpoint named '/activity' created at bacend for contracts
        loadActivityFeed(),
        // TEMP
        // REASON: Not consu,ing anywhere in FE
        // loadUserData(),
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  ...
                  <div>
                    ...
                    >
                      {getFileSize(contract.contractFile.fileSize)} •{' '}
                      {contract.contractFile.mimeType?.split('/')[1]?.toUpperCase()}
                    </div>
                  </div>
                </div>
          ...
          >
            ...
            >
              <div style={{ textAlign: 'center' }}>
                <div
                  style={{
                    ...
                    height: '8rem',
                    borderRadius: '50%',
                    backgroundColor: getRiskLevelColor(contract?.analysis?.riskLevel) + '20',
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                    margin: '0 auto 1rem',
                ...
                <div
                  style={{
                    ...
                    padding: '0.5rem 1rem',
                    backgroundColor: getRiskLevelColor(contract.analysis?.riskLevel) + '20',
                    color: getRiskLevelColor(contract.analysis?.riskLevel),
                    borderRadius: '9999px',
                    fontSize: '0.875rem',
                    fontWeight: '600',
                    textTransform: 'capitalize',
              ...
              <div>
                ...
                >
                  {contract.analysis?.riskScore <= 30
                    ...
                    : contract.analysis?.riskScore <= 60
                      ...
                      : contract.analysis?.riskScore <= 80
                    ...
                    >
                      ...
                      <div
                        style={{
                          padding: '0.125rem 0.5rem',
                          backgroundColor:
                            point.priority === 'must_have'
                              ? '#EF4444'
                              : point.priority === 'important'
                                ? '#F59E0B'
                                : '#10B981',
                          color: '#FFFFFF',
                          borderRadius: '9999px',
