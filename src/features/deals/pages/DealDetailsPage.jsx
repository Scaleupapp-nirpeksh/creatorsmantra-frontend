...
const DealDetailsPage = () => {
  ...
  const formatCurrency = (amount) =>
    new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR', minimumFractionDigits: 0 }).format(Number.isFinite(amount) ? amount : 0);
  ...
  const calculateDealHealth = () => {
    ...
    if (deal.timeline?.paymentDueDate && new Date(deal.timeline.paymentDueDate) < now && deal.stage !== 'paid') score -= 20;
  ...
  const styles = {
    ...
    headerTop: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' },
    backButton: { display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem', background: '#fff', border: '1px solid #e2e8f0', borderRadius: '0.5rem', color: '#475569', fontSize: '0.875rem', cursor: 'pointer' },
    titleSection: { flex: 1, margin: '0 2rem' },
    title: { fontSize: '1.75rem', fontWeight: 700, color: '#0f172a', marginBottom: '0.25rem' },
    subtitle: { fontSize: '0.875rem', color: '#64748b', display: 'flex', alignItems: 'center', gap: '1rem' },
    actions: { display: 'flex', gap: '0.75rem', position: 'relative' },
    button: { display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.625rem 1rem', borderRadius: '0.5rem', fontSize: '0.875rem', fontWeight: 500, cursor: 'pointer', border: 'none', transition: '0.2s' },
    primaryButton: { background: '#6366f1', color: '#fff' },
    secondaryButton: { background: '#fff', color: '#475569', border: '1px solid #e2e8f0' },
    dropdown: { position: 'absolute', top: '100%', right: 0, marginTop: '0.5rem', background: '#fff', border: '1px solid #e2e8f0', borderRadius: '0.5rem', boxShadow: '0 4px 6px rgba(0,0,0,0.1)', zIndex: 50, minWidth: 200 },
    dropdownItem: { display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.625rem 1rem', fontSize: '0.875rem', color: '#475569', cursor: 'pointer' },

    stageProgress: { display: 'flex', alignItems: 'center', gap: '1rem', padding: '1rem 0', overflowX: 'auto' },
    stageItem: { display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem', borderRadius: '2rem', fontSize: '0.875rem', fontWeight: 500, cursor: 'pointer', whiteSpace: 'nowrap', transition: '0.2s' },
    stageActive: { background: currentStage?.color || '#8B5CF6', color: '#fff' },
    stageInactive: { background: '#f1f5f9', color: '#64748b' },

    statsRow: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: '1rem', marginTop: '1rem' },
    statCard: { background: '#fff', padding: '1rem', borderRadius: '0.5rem', border: '1px solid #e2e8f0' },
    statLabel: { fontSize: '0.75rem', color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.25rem' },
    statValue: { fontSize: '1.25rem', fontWeight: 700, color: '#0f172a', display: 'flex', alignItems: 'center', gap: '0.5rem' },

    content: { display: 'flex', gap: '1.5rem', padding: '1.5rem', maxWidth: 1400, margin: '0 auto' },
    mainColumn: { flex: '1 1 65%' },
    sideColumn: { flex: '1 1 35%' },

    card: { background: '#fff', borderRadius: '0.75rem', border: '1px solid #e2e8f0', marginBottom: '1.5rem', overflow: 'hidden' },
    cardHeader: { padding: '1.25rem', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
    cardTitle: { fontSize: '1rem', fontWeight: 600, color: '#0f172a', display: 'flex', alignItems: 'center', gap: '0.5rem' },
    cardContent: { padding: '1.25rem' },

    tabs: { display: 'flex', gap: '0.5rem', padding: '0 1.25rem', borderBottom: '1px solid #e2e8f0' },
    tab: { padding: '0.875rem 0', fontSize: '0.875rem', fontWeight: 500, color: '#64748b', cursor: 'pointer', borderBottom: '2px solid transparent' },
    tabActive: { color: '#6366f1', borderBottomColor: '#6366f1' },

    field: { marginBottom: '1rem' },
    fieldLabel: { fontSize: '0.75rem', color: '#64748b', fontWeight: 500, textTransform: 'uppercase', marginBottom: '0.25rem' },
    fieldValue: { fontSize: '0.9375rem', color: '#0f172a' },
    input: { width: '100%', padding: '0.625rem', border: '1px solid #e2e8f0', borderRadius: '0.375rem', fontSize: '0.9375rem' },
    textarea: { width: '100%', padding: '0.625rem', border: '1px solid #e2e8f0', borderRadius: '0.375rem', fontSize: '0.9375rem', minHeight: 100, resize: 'vertical' },

    communicationItem: { padding: '1rem', borderLeft: '3px solid #e2e8f0', marginBottom: '1rem' },

    loadingContainer: { display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 400, gap: '1rem' },
    spinner: { width: 40, height: 40, border: '3px solid #e2e8f0', borderTop: '3px solid #6366f1', borderRadius: '50%', animation: 'spin 1s linear infinite' },
    errorContainer: { display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 400, gap: '1rem' },
  ...
  const validateComm = (c) => {
    const errs = {};
    if (!c.summary || c.summary.trim().length < 5) errs.summary = 'Summary must be at least 5 characters';
    if (!c.nextAction || !c.nextAction.trim()) errs.nextAction = 'Next action is required';
    if (!c.followUpDate || isNaN(new Date(c.followUpDate).getTime())) errs.followUpDate = 'Follow-up date must be valid';
    return errs;
  };
  ...
  if (!deal) {
    return (
      <div style={styles.errorContainer}>
        ...
        <button onClick={() => navigate('/deals')} style={styles.button}>Back to Deals</button>
      </div>
    );
  }
    ...
    <div style={styles.container}>
      ...
      <div style={styles.header}>
              ...
              <>
                ...
                {showActions && (
                  <div style={styles.dropdown}>
                    <div className="dropdown-item" style={styles.dropdownItem} onClick={() => handleQuickAction('duplicate')}>
                      <Copy size={16} /> Duplicate Deal
                    </div>
                    <div className="dropdown-item" style={styles.dropdownItem} onClick={() => handleQuickAction('convert_to_template')}>
                      <FileText size={16} /> Convert to Template
                    </div>
                    <div className="dropdown-item" style={styles.dropdownItem} onClick={() => handleQuickAction('send_reminder')}>
        ...
        <div style={styles.stageProgress}>
          {stages.filter(s => !['cancelled', 'rejected'].includes(s.id)).map(stage => {
            ...
            const isPast = stages.findIndex(s => s.id === deal.stage) > stages.findIndex(s => s.id === stage.id);
            return (
              <div
                key={stage.id}
                style={{
        ...
        <div style={styles.statsRow}>
          ...
          <div style={styles.statCard}>
            ...
            <div style={styles.statValue}><Globe size={18} />{deal.platform || 'Not set'}</div>
          </div>
          <div style={styles.statCard}>
            <div style={styles.statLabel}>Priority</div>
            <div style={styles.statValue}><Zap size={18} />{deal.priority || 'Medium'}</div>
          </div>
          <div style={styles.statCard}>
            <div style={styles.statLabel}>Health Score</div>
            <div style={styles.statValue}>{dealHealth}%</div>
            <div style={{ height: 6, background: '#e2e8f0', borderRadius: 3, overflow: 'hidden', marginTop: '0.5rem' }}>
              <div style={{ height: '100%', width: `${dealHealth}%`, background: dealHealth > 70 ? '#10b981' : dealHealth > 40 ? '#f59e0b' : '#ef4444', transition: 'width .3s ease' }} />
            </div>
          </div>
        </div>
      </div>
              ...
              {activeTab === 'deliverables' && (
                <div>
                  {(deal.deliverables || []).map((deliverable, index) => (
                    <div key={index} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem', background: '#f8fafc', borderRadius: '0.5rem', marginBottom: '0.5rem' }}>
                      <div>
                        <div style={{ fontWeight: 600 }}>{deliverable.type}</div>
                        <div style={{ fontSize: '0.875rem', color: '#64748b' }}>
                          Qty: {deliverable.quantity} {deliverable.description ? `| ${deliverable.description}` : ''}
                        </div>
                      </div>
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            if (status === 'submitted') {
                              ...
                              if (url) handleUpdateDeliverable(deliverable._id, status, { submissionUrl: url });
                            } else if (status === 'revision_required') {
                              const notes = window.prompt('Enter revision notes:');
                              if (notes) handleUpdateDeliverable(deliverable._id, status, { revisionNotes: notes });
                            } else {
                  ...
                  {(!deal.deliverables || deal.deliverables.length === 0) && (
                    <div style={{ textAlign: 'center', padding: '2rem', color: '#64748b' }}>No deliverables added</div>
                  )}
                </div>
              )}
              ...
              {activeTab === 'communications' && (
                <div>
                  <button
                    ...
                    onClick={() => { setCommErrors({}); setShowAddComm(true); }}
                  >
                  ...
                  {communications.map((comm, index) => (
                    <div key={index} style={styles.communicationItem}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                        ...
                        <span style={{ fontSize: '0.75rem', color: '#64748b' }}>{formatDate(comm.createdAt)}</span>
                      </div>
                      <div style={{ fontSize: '0.875rem', marginBottom: '0.5rem' }}>{comm.summary}</div>
                  ...
                  {communications.length === 0 && (
                    <div style={{ textAlign: 'center', padding: '2rem', color: '#64748b' }}>No communications recorded</div>
                  )}
                </div>
              )}
                ...
                <div>
                  <div style={styles.field}>
                    ...
                    {editMode ? (
                      ...
                      <div style={styles.fieldValue}>{formatDate(deal.timeline?.responseDeadline)}</div>
                    )}
                  </div>
                  ...
                  <div style={styles.field}>
                    ...
                    {editMode ? (
                      ...
                      <div style={styles.fieldValue}>{formatDate(deal.timeline?.contentDeadline)}</div>
                    )}
                  </div>
                  ...
                  <div style={styles.field}>
                    ...
                    {editMode ? (
                      ...
                      <div style={styles.fieldValue}>{formatDate(deal.timeline?.paymentDueDate)}</div>
                    )}
                  </div>
                </div>
              )}
        ...
        <div style={styles.sideColumn}>
          ...
          <div style={styles.card}>
            <div style={styles.cardHeader}>
              <div style={styles.cardTitle}><Building size={18} /> Brand Details</div>
            </div>
            <div style={styles.cardContent}>
              <div style={styles.field}>
              ...
              <div style={styles.field}>
                ...
                {editMode ? (
                  ...
                  <div style={styles.fieldValue}>{deal.brand?.contactPerson?.name || 'Not specified'}</div>
                )}
              </div>

              <div style={styles.field}>
                <div style={styles.fieldLabel}>Email</div>
                <div style={styles.fieldValue}>{deal.brand?.contactPerson?.email || 'Not specified'}</div>
              </div>

              <div style={styles.field}>
                <div style={styles.fieldLabel}>Phone</div>
                <div style={styles.fieldValue}>{deal.brand?.contactPerson?.phone || 'Not specified'}</div>
              </div>
            </div>
          </div>
          ...
          <div style={styles.card}>
            <div style={styles.cardHeader}>
              <div style={styles.cardTitle}><CreditCard size={18} /> Payment Details</div>
            </div>
            <div style={styles.cardContent}>
              <div style={styles.field}>
              ...
              <div style={styles.field}>
                ...
                {editMode ? (
                  ...
                  <div style={styles.fieldValue}>{deal.dealValue?.gstApplicable ? 'Yes (18%)' : 'No'}</div>
                )}
              </div>
              ...
              <div style={styles.field}>
                ...
                {editMode ? (
                  ...
                  <div style={styles.fieldValue}>{deal.dealValue?.tdsApplicable ? 'Yes (10%)' : 'No'}</div>
                )}
              </div>
              ...
              <div style={styles.field}>
                ...
                <div style={styles.fieldValue}>
                  {(() => {
                    const base = sanitizeNumber(editMode ? editedDeal.dealValue?.amount : deal.dealValue?.amount, 0);
                    const gst = (editMode ? editedDeal.dealValue?.gstApplicable : deal.dealValue?.gstApplicable) ? base * 0.18 : 0;
                    const tds = (editMode ? editedDeal.dealValue?.tdsApplicable : deal.dealValue?.tdsApplicable) ? base * 0.10 : 0;
      ...
      {showStageModal && (
        <div style={{
          ...
          <div style={{ background: '#fff', borderRadius: '0.75rem', padding: '1.5rem', maxWidth: 500, width: '90%' }}>
            <h3 style={{ fontSize: '1.125rem', fontWeight: 600, marginBottom: '1rem' }}>Update Deal Stage</h3>
            <div style={styles.field}>
              ...
              <div style={{
                ...
                background: getStageInfo(selectedStage).color, color: '#fff'
              }}>
            ...
            <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end', marginTop: '1.5rem' }}>
              <button
                style={{ ...styles.button, ...styles.secondaryButton }}
                onClick={() => { setShowStageModal(false); setSelectedStage(null); setStageNotes(''); }}
              >
                Cancel
              </button>
              <button
      ...
      {showAddComm && (
        <div style={{
          ...
          <div style={{ background: '#fff', borderRadius: '0.75rem', padding: '1.5rem', maxWidth: 560, width: '92%' }}>
            <h3 style={{ fontSize: '1.125rem', fontWeight: 600, marginBottom: '1rem' }}>Add Communication</h3>
            ...
            <div style={styles.field}>
              ...
              <input
                ...
                style={styles.input}
              />
              {commErrors.followUpDate && <div style={styles.errorText}>{commErrors.followUpDate}</div>}
            </div>
