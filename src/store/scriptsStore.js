    ...
    (set, get) => ({
      ...
      createVideoScript: async (scriptData, videoFile) => {
        try {
          ...
          const response = await scriptsAPI.createScriptWithFile(
            scriptData,
            videoFile,
            'video_transcription'
          )
          ...
          if (response.success) {
            ...
            set((state) => ({
              scripts: state.scripts.map((script) =>
                script.id === scriptId ? { ...script, ...updateData } : script
              ),
            }))
      ...
      bulkUpdateStatus: async (status, reason = null) => {
        try {
          ...
          if (response.success) {
            ...
            set((state) => ({
              scripts: state.scripts.map((script) =>
                selectedScripts.includes(script.id) ? { ...script, status } : script
              ),
              selectedScripts: [],
            }))
