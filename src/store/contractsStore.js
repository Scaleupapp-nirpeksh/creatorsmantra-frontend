    ...
    (set, get) => ({
        ...
        try {
          ...
          if (response.success) {
            ...
            set({
              ...
              totalContracts: pagination.totalContracts,
      ...
      uploadContract: async (formData) => {
        try {
          ...
          const response = await contractsAPI.uploadContract(formData, (progress) => {
      ...
      fetchUploadLimits: async () => {
        // TEMP
        // REASON: No API Built in backend to fetch upload-limits
        set({
          uploadLimits: {
            canUpload: true,
            currentUsage: 2,
      ...
      updateContractStatus: async (contractId, status, notes = '') => {
        try {
          ...
          if (response.success) {
            ...
            const updatedCurrentContract =
              currentContract?.id === contractId
        ...
        try {
          ...
          if (response.success) {
            ...
            const currentContract =
              get().currentContract?.id === contractId ? null : get().currentContract
      ...
      init: async () => {
        try {
          ...
          // TEMP
          // No data for dashboardAnalytics & Upload functionality breaking
